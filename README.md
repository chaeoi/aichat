...
- **编辑与重新生成**：编辑任意用户消息后从该处重发；任意助手回复可重新生成
- **Markdown 渲染**：代码块（带复制按钮）、表格、标题、列表、引用、链接、粗斜体，渲染前转义防 XSS
- **会话管理**：新建（首条消息才落库，不产生空会话）、重命名、删除、侧栏搜索
- **智能标题**：首轮对话结束后由模型自动总结会话标题，通过 SSE 实时推送到侧栏
...
| `DELETE` | `/api/sessions/{id}/messages/{mid}` | 删除该消息及其之后的所有消息（编辑重发/重新生成用） |
| `POST` | `/api/chat/stream` | `{session_id, content, model, provider?, regenerate?}` → SSE 流式回复。会话不存在时自动创建；上下文由服务端构建；`regenerate: true` 时基于现有历史重新生成。首轮结束后会推送 `event: title` 事件（模型总结的标题） |
...
- [x] **重新生成回复**：任意助手消息可重试（后端支持消息截断后重新推理）
- [x] **编辑已发送消息**：编辑用户消息并从该处重新生成
- [ ] **回答分支**：编辑/重新生成时保留旧版本，左右切换分支（ChatGPT 的 `< 2/2 >`；当前实现会覆盖旧回答）
- [ ] **代码语法高亮**：接入 highlight.js 或 Shiki（当前代码块仅等宽展示）
- [x] **Markdown 表格**
- [ ] **Markdown 补齐**：任务列表、嵌套列表、LaTeX 数学公式（KaTeX）
- [ ] **推理模型支持**：解析 `reasoning_content` 思维链，折叠展示"思考过程"（DeepSeek-R1 / o 系列必需）
- [x] **LLM 自动生成标题**：首轮对话后用模型总结标题，SSE 实时推送
- [ ] **每会话参数设置**：系统提示词、temperature、max_tokens 面板
- [ ] **流式断线重连/请求重试**：网络抖动时不丢回复
- [x] **对话搜索**：侧边栏按标题即时过滤（全文搜索待做）
